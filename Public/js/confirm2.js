var myScroll,myScrol2,myScrol3,myScrol4,
...
$(document).ready(function() {
    ...
    mod_cart();
    returnBth();
    function init(){divs.show();}
    function iscrollAll(){
      myScroll = new IScroll('#mod_confirm-container',lists);
      myScrol4 = new IScroll('#mod_coupons', lists);
  /*    myScrol3 = new IScroll('#mod_pay_dist', lists);*/
    ...
    function mod_cart(){
      var waydiv=$('.mod_dir_way>div'),
        ...
        $('.confrim_goods_coupons').on(tapClick(),function(){
            $('#mod_pay_coupons').show().siblings().hide();
             iscrollAll(); 
        })
        waydiv.off(tapClick());
    ...
    function returnBth(){
        $('.icon-back,.icon-me').on(tapClick(),function(){
            ...
            if(parentName.indexOf('mod_confirm_order')>-1){
               ...
               iscrollAll();
            }else if(parentName.indexOf('mod_pay_distribution')>-1){ 
              var idx = $('.mod_dir_way .checks').attr('data-index'),
                  txt= "";
                  txt = $('.mod_dir_way .checks').val();
                ...
                if(idx){
                  ...
                  if($('.mod_pay_uls li span').hasClass('spans') && idx == 2){
                    var ind = ($('.spans').parents('.mod_show').attr('data-index'));
                    if(ind == 1){
                      txt = $('#book_day option:checked').text() +' '+ $('.spans').text();
                    }else{
                      txt = $('.mod_pay_ps').text() +' '+ $('.spans').text();
                    }
                    
                    $("input[name='hide-times']").val(txt);
                  }else if(idx != 2){
                     txt = $('.mod_dir_way .checks').val();
                     $("input[name='hide-times']").val("");
                  }else{
                ...
                $('.confrim_time').html(txt);
                var is_voucher_check=Number($('input[name=is_voucher_check]').val()),
                    goods_amounts = Number($('#goods_amounts').val()),
                    post_fees = Number($('#post_fees').val()),
                    money = Number($('input[name=is_voucher_check]').attr('money'));
                if($('.mod_dir_way .checks').val() == '到店自提' && idx == 1){
                  var goods_money=0;
                  if (is_voucher_check) {
                    goods_money=goods_amounts - money;
                  }else{
                    goods_money=goods_amounts;
                  }
                  if (goods_money < 0) {
                    goods_money=0;
                  }
                  ...
                  $('.amount_price b').text(goods_money);
                  $('#size_price b').text(goods_money);
                  $('.peisong b').text('0');
                }else{
                  if (is_voucher_check) {
                    goods_money=goods_amounts - money + post_fees;
                  }else{
                    goods_money=goods_amounts + post_fees;
                  }
                  if (goods_money < 0) {
                    goods_money=0;
                  }
