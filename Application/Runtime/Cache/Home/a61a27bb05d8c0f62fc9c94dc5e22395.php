	...
	<div class="wrap_details wrap_de">
		<div class="wrap_deSize">
			...
			<div class="wrap_deMenu flex">
				<span class="d_span">预约截止时间：</span>
				<p class="flexs">12月14日中午11:00</p>
			</div>
			<div class="wrap_deMenu flex">
				<span class="d_span">配送时间：</span>
				<p class="flexs"><?php echo ($xianshi_data['hou_time']); ?>日至<?php echo ($xianshi_data['dahouhou_time']); ?>日，11:00—22:00</p>
			</div>
			<div class="wrap_deMenu flex">
		...
		<div class="wrap_deInfo">
			<div class="wrap_deITime" >
				<p class="deITime" id="timer" data-timer="<?php echo ($end_time); ?> 11:00:00">
					<!--<span id="timer_d">00</span>:--><span id="timer_h">00</span>:<span id="timer_m">00</span>:<span id="timer_s">00</span>
				</p>
			</div>
			...
			<div class="wrap_sp flex">
				...
				<div class="wrap_J flex">
					<!--<span class="product-operates-item del tap-action" id="del">
					    <span class="inner theme-spline">
					    <?php if(($cart_goods_count) == "0"): ?><img src="/Public/image/del_icon.png" alt="">
					    <?php else: ?>
					    <img src="/Public/image/del_icons.png" alt=""><?php endif; ?> 
					    <img src="/Public/image/del_icon.png" alt="">
					    </span>
					</span> -->
					<span id="amount" class="product-operates-item amount amountInp"><?php echo ($cart_goods_count); ?>份</span>
					<!--<span class="product-operates-item add tap-action" id="add">
					    <span class="inner theme-spline">
					    <?php if(($cart_goods_count) == "1"): ?><img src="/Public/image/addhui_icon.png" alt="">
					    <?php else: ?>
						<img src="/Public/image/add_icons.png" alt=""><?php endif; ?>
					    </span>
					</span> -->
				</div>
			</div>	
			<form id="autocomplete-form" class="mod-order-common login-form" onsubmit="return false;"  action="/ActivityTwo/confirm">
		...
		<div class="wrap_de_btn">
			<!-- <img src="/Public/image/active/d_btn.png" alt=""> -->
...
<script type="text/javascript">
		...
		$(function(){
			...
			// 			loaging.close();
			// 			if(res.result == 'ok'){
			// 				book_ticket_id.val(res.arr.book_ticket_id);
			// 				cart_id.val(res.arr.cart_id);
			// 				if(res.arr.cart_goods_count <= 0 ){
			// 					that.find('img').attr('src','/Public/image/del_icon.png');						
			// 				}

			// 				add.find('img').attr('src','/Public/image/add_icons.png');
			// 				amount.html(res.arr.cart_goods_count);
			// 			}

			// 		},error:function(){
			// 			loaging.close();
			// 			loaging.btn('添加失败，请刷新重试');
			// 		}
			// 	});
			// });
			// add.off(tapClick());
			// add.on(tapClick(),function(){
			// 	loaging.close();
			// 	loaging.init('加载中，请稍后...');
			// 	var amountVal = amount.html(),
			// 		that = $(this);
			...
			// 					cart_id.val(res.arr.cart_id);
			// 					amount.html(res.arr.cart_goods_count);
			// 					if(res.arr.cart_goods_count >= 1){
			// 						that.find('img').attr('src','/Public/image/addhui_icon.png');
			// 					}
			// 					del.find('img').attr('src','/Public/image/del_icons.png');
			// 				}
	        ...
	        function timer(eleId){  
	            var element = $('#timer');  
	            if(element){  
	                endTimer=element.attr('data-timer');  
	                var endTime = new Date(endTimer).getTime();
	                ...
	                if(ts>0){  
	                   ...
	                   // document.getElementById("timer_d").innerHTML=dd;  
	                    ...
	                    clearTimeout(function(){timer(eleId);});
	                }  
	            }
			}
		});
