...
<div id="mod-container" class="mod-container clearfix">
    <div id="mod-success" class="mod-success pageview">
        <header class="pub-header">
            <a class="tap-action icon icon-back" href="/Address/index.html"></a>
...
var myScroll,citys=GetQueryString('cityName'),map = new BMap.Map("l-map");
		...
		if(data){
			var addComp = data.result,
				...
				setTimeout(function(){
					...
					var circle = new BMap.Circle(pp,200,{strokeColor:"blue", strokeWeight:1.5, strokeOpacity:0.8});
				...
				var html='<h2><img src="/Public/image/positio_icon.png" alt=""><span class="default_city">'+cityName+regionName+streetnName+'</span></h2>'
						...
						for(var i=0, len=addComp.pois.length; i<len; i++){
							...
							html+='<li><a href="/Address/setsession_data.html?district='+arr.name+'&x='+k+'&y='+s+'&cityCode='+cityCode+'">'
									+'<h3>'+arr.name+'</h3>'
									+'<p>'+arr.addr+'</p>'
									+'</a></li>'
						}
