...
class Jpush{
    ...
    */
    ...
    public function push($receiver='all',$content='',$m_type='',$m_txt='',$m_time='86400'){
		...
		*/
        ...
        if($res){//得到返回值--成功已否后面判断
			...
			if($res_arr['error']['code'])
				$retcode = intval($res_arr['error']['code']);
			
			return $this->getErrorMessage($retcode);
        }else{//未得到返回值--返回失败
            return false;
        }
    }
	...
	public function shopPush($receiver='all',$content='', $platform=1, $m_type='', $m_txt='', $m_time='86400', $sound = 'default'){
        $base64=base64_encode("$this->app_key:$this->master_secret");
        $header=array("Authorization:Basic $base64","Content-Type:application/json");
        $data = array();
		...
		$data['platform'] = $plat[$platform];//目标用户终端手机的平台类型android,ios,winphone
        $data['audience'] = $receiver;      //目标用户
    ...
    public function push_curl($param="",$header="") {
        ...
        curl_setopt($ch, CURLOPT_POSTFIELDS, $curlPost);        //传送费时
		//curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query($curlPost)); //传送省时
	...
	public function getErrorMessage($errcode){
		...
		switch ($errcode) {
			...
			case 10:
				...
				break;
			case 1001:
			    $errmsg = '只支持 HTTP Post 方法，不支持 Get 方法';
			...
			case 1011:
				...
				break;
			case 1012:
				$errmsg = 'iOS 不支持推送自定义消息。只有 Android 支持推送自定义消息';
