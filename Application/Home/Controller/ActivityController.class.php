...
class ActivityController extends CommonController {
    ...
    public function test111(){
        ...
        $LinshiActivety_data=$LinshiActivety->getOpenid($openId);
        ...
        //查询活动产品数量是否够用
        $goods_is_max=self::is_stock_max();
    ...
    public function order_add(){
        ...
        if ($return_user_data['result'] == 'ok') {
            ...
            $cart_data['count']='1';
            $rester_cart=self::activity_cart_add($cart_data,$user_data['user_id'],$user_data['token']);
            if (!$rester_cart) {
                echo json_encode(array('result'=>'error','msg'=>'产品已售卖光'));
                exit();
            }
            ...
            $order_data['cart_ids']=$rester_cart['cart_id'];
            ...
            if ($rester_arr['result']=='1') {
                $LinshiActivety = D('Home/LinshiActivety');
                $LinshiActivety->add_data(array('openId'=>$openId,'addtime'=>time(),'oreder_no'=>$rester_arr['data']['order_no']));
                 //微信支付
    ...
    //获取活动产品库存是否够用
    public function is_stock_max(){
        $url=API_URL.'/rest_2/cart/is_stock_max';
        $post_data = array();
    ...
    //根据订单编号删除对应的openid
    public function del_openId(){
        $oreder_no=$_REQUEST['oreder_no'];
        $LinshiActivety = D('Home/LinshiActivety');
        $LinshiActivety->delOpenid($oreder_no);
    }
}
?>