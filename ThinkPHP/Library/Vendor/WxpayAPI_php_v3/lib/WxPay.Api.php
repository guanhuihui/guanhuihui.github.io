...
class WxPayApi
{
	...
	//企业向个人返款
		public static function unifiedShop($inputObj, $timeOut = 6)
	{
		...
		$result = WxPayResults::Up_Init($response);
		self::reportCostTime($url, $startTimeStamp, $result);//上报请求花费时间
	...
	{		
		...
		if($useCert == true){
			...
			curl_setopt($ch,CURLOPT_SSLCERTTYPE,'PEM');
			curl_setopt($ch,CURLOPT_SSLCERT, VENDOR_PATH.'/WxpayAPI_php_v3/cert/apiclient_cert.pem');
			curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
			curl_setopt($ch,CURLOPT_SSLKEY, VENDOR_PATH.'/WxpayAPI_php_v3/cert/apiclient_key.pem');
		}
		//post提交方式
